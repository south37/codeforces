...
int main(int argc, char** argv) {
  ...
  ll cnt = 0; // count of not fixed. They are candidates.
  rep(i,n) {
    if (a[i] >= 0) { continue; } // already fixed
    if (st.find(i) == st.end()) { // not used now.
      ++cnt;
    }
  }

  Combination comb(2005);
  mint ans = 0;
  rep(i,cnt+1) { // loop fixed count in [0,cnt]
    mint now = comb(cnt,i); // select fix location
    now *= comb.fact[n-st.size()-i];
    if (i%2==1) {
      now *= -1;
    }
    ans += now;
